...
class UARTParser():
        ...
        if (self.saveBinary == 1):
            ...
            if (self.uartCounter % self.framesPerFile == 0):
                if (self.first_file is True):
                    ...
                    os.mkdir('binData/' + self.filepath)
                    self.first_file = False
                with open('./binData/' + self.filepath + '/replay_' + str(
                        math.floor(self.uartCounter / self.framesPerFile)) + '.json', 'w') as fp:
    ...
    def connectComPorts(self, cliCom, dataCom):
        self.cliCom = serial.Serial(cliCom, 115200, parity=serial.PARITY_NONE, stopbits=serial.STOPBITS_ONE,
                                    timeout=0.6)
    ...
    def connectComPort(self, cliCom, cliBaud=115200):
        ...
        self.cliCom = serial.Serial(cliCom, cliBaud, parity=serial.PARITY_NONE, stopbits=serial.STOPBITS_ONE,
                                    timeout=.6)
    ...
    def sendCfg(self, cfg):
        ...
        for line in cfg:
            ...
            if (self.cliCom.baudrate == 1250000):
                for char in [*line]:
                    time.sleep(
                        .001)  # Character delay. Required for demos which are 1250000 baud by default else characters are skipped
            ...
            if (splitLine[
                0] == "baudRate"):  # The baudrate CLI line changes the CLI baud rate on the next cfg line to enable greater data streaming off the xWRL device.
                try:
...
if __name__ == "__main__":
    p = UARTParser(type="DoubleCOMPort")
    p.parserType = "DoubleCOMPort"
    self.core.connectCom(self.cliCom, self.dataCom, self.connectStatus)

    p.readAndParseUartDoubleCOMPort()